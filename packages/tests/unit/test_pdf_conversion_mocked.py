...
class TestPDFConversionMocked:
    ...
    def test_weasyprint_called_with_html_content(self, mock_html_class, minimal_audit_results, tmp_path):
    ...
    @patch("glassalpha.report.renderers.pdf.HTML")
    def test_weasyprint_write_pdf_called(self, mock_html_class, tmp_path):
        ...
        assert output_path in call_args[0] or str(output_path) in str(call_args)

    @patch("glassalpha.report.renderers.pdf.HTML")
    def test_pdf_conversion_creates_output_file(self, mock_html_class, tmp_path):
        ...
        mock_html_instance.write_pdf.side_effect = create_dummy_pdf
    ...
    @patch("glassalpha.report.renderers.pdf.HTML")
    def test_pdf_conversion_handles_import_error(self, mock_html_class, tmp_path):
    ...
    @patch("glassalpha.report.renderers.pdf.HTML")
    def test_pdf_conversion_handles_runtime_error(self, mock_html_class, tmp_path):
        ...
        mock_html_instance = Mock()
        mock_html_class.return_value = mock_html_instance
        mock_html_instance.write_pdf.side_effect = RuntimeError("PDF generation failed")
    ...
    @patch("glassalpha.report.renderers.pdf.HTML")
    def test_render_audit_pdf_end_to_end_mocked(self, mock_html_class, minimal_audit_results, tmp_path):
        ...
        result = render_audit_pdf(minimal_audit_results, output_path)
    ...
    @patch("glassalpha.report.renderers.pdf.HTML")
    def test_pdf_conversion_with_stylesheets(self, mock_html_class, tmp_path):
        ...
        # Verify stylesheets were passed (if any)
        mock_html_instance.write_pdf.assert_called_once()
        call_kwargs = mock_html_instance.write_pdf.call_args[1]

        # Stylesheets might be passed as keyword argument
...
class TestPDFMetadata:
    ...
    def test_pdf_metadata_set(self, mock_set_metadata, mock_html_class, tmp_path):
...
class TestPDFErrorHandling:
    ...
    @patch("glassalpha.report.renderers.pdf.HTML")
    def test_pdf_handles_invalid_html(self, mock_html_class, tmp_path):
    ...
    @patch("glassalpha.report.renderers.pdf.HTML")
    def test_pdf_handles_write_permission_error(self, mock_html_class, tmp_path):
        """PDF conversion should handle write permission errors."""
        output_path = tmp_path / "readonly" / "test.pdf"

        mock_html_instance = Mock()
        mock_html_class.return_value = mock_html_instance
        mock_html_instance.write_pdf.side_effect = PermissionError("Cannot write")
